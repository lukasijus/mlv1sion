...
async def github_callback(
    ...
    redirect_target = svc.decode_github_state(state)

    if error:
        failure_url = svc.build_github_error_redirect(
            redirect_target,
            error=error,
            description=error_description,
        )
    ...
    if not code:
        failure_url = svc.build_github_error_redirect(
            redirect_target,
            error="missing_code",
            description="GitHub did not return an authorization code",
        )
        return RedirectResponse(failure_url, status_code=status.HTTP_302_FOUND)

    tokens = await svc.login_with_github_code(code)
    success_url = svc.build_github_success_redirect(redirect_target, tokens)
    return RedirectResponse(success_url, status_code=status.HTTP_302_FOUND)
